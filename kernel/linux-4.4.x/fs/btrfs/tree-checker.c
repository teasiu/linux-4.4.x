...
static void fix_extent_item_key(struct btrfs_fs_info *fs_info, struct extent_buffer *leaf, int slot, struct btrfs_key *bad_key)
{
	...
	btrfs_warn(root->fs_info, "[try to fix] corrupt leaf, bad extent item key, "
...
static void fix_node_blockptr(struct btrfs_root *root, struct extent_buffer *node, int slot, struct btrfs_key *key)
{
	...
	// get the key of first child
	eb = read_tree_block(root, fixed_bytenr, generation, btrfs_header_level(node) - 1, NULL);
	if (IS_ERR(eb))
		goto restore_key;
	else if (!extent_buffer_uptodate(eb))
		goto free_eb;
...
int btrfs_check_node(struct extent_buffer *node)
{
	...
	if (nr == 0 || nr > BTRFS_NODEPTRS_PER_BLOCK(fs_info->tree_root)) {
		btrfs_crit(fs_info,
#ifdef MY_ABC_HERE
"[cannot fix] corrupt node: root=%llu block=%llu, nritems too %s, have %lu expect range [1,%zu]",
#else
