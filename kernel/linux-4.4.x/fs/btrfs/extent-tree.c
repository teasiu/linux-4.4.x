				  ...
				  struct btrfs_delayed_extent_op *extent_op)
{
	...
	if (!fs_info->syno_quota_v1_enabled || !is_fstree(root_objectid))
		no_quota = 1;
#endif /* MY_ABC_HERE */

#ifdef MY_ABC_HERE
	if (!no_quota && is_fstree(root_objectid)) {
...
static int __btrfs_alloc_data_chunk_ondemand(struct inode *inode, u64 bytes
#ifdef MY_ABC_HERE
											 , bool commit
#endif /* MY_ABC_HERE */
											 )
...
{
	return __btrfs_alloc_data_chunk_ondemand(inode, bytes,
#ifdef MY_ABC_HERE
											 true
#endif /* MY_ABC_HERE */
											 );
...
{
	return __btrfs_alloc_data_chunk_ondemand(inode, bytes, false);
}
...
out:
	...
	{
		// This could happen due to userspace stopping cleaner
